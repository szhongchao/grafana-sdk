...
func TestMarshalAndUnmarshal_GrafanaV5Dashboard(t *testing.T) {
	var board sdk.Board
	raw, _ := ioutil.ReadFile("testdata/dashboard-grafana-5.0.json")
	...
	outJSON, err := json.MarshalIndent(board, "", "\t")

	if len(outJSON) == 0 {
		t.Fatal("Dashboard marshalled to empty json")
	}

	if err != nil {
		t.Fatal("error marshalling dashboard: ", err)
	}
}
